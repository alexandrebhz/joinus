...
type CrawledJob struct {
	...
	CustomFields      map[string]string  `json:"custom_fields"`   // Dynamic fields that don't map to standard fields
...
func (cj *CrawledJob) GetCustomField(key string) (string, bool) {
	if cj.CustomFields == nil {
		return "", false
	}
	value, exists := cj.CustomFields[key]
	return value, exists
}

// SetCustomField sets a custom field value
func (cj *CrawledJob) SetCustomField(key, value string) {
	if cj.CustomFields == nil {
		cj.CustomFields = make(map[string]string)
	}
	cj.CustomFields[key] = value
}
